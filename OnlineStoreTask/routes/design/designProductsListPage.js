    ...
    _.each(filteredProducts, (product) => {
      ...
      if (colorsObj) {
        ...
        _.each(product.image_groups, (object) => {
          ...
          && _.contains(images.swatch, object.images[0].link) === false) {
            images.swatch.push(object.images[0].link);
          }
        });
      }
      ...
      const mypr = Object.assign({}, product);
      mypr.images = JSON.parse(JSON.stringify(images));
      mypr._doc.images = JSON.parse(JSON.stringify(images));
      Object.assign(product, mypr);
